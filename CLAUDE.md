...
- **Billing:** Stripe (SaaS mode only, conditional on STRIPE_SECRET_KEY)
- **Testing:** Vitest
...
# Tests
cd backend && npm test                             # Vitest — 131 tests
...
## Multi-Tenancy (v2)
- Row-level `tenant_id` on all tenant-sensitive tables
- `tenants` table with slug, plan (free/pro/enterprise/self_hosted), settings
- JWT includes `tenant_id`, `is_superadmin`
- `tenantHook` resolves tenant from JWT → X-Tenant-Slug header
- Billing routes conditional on `SAAS_MODE=true` or `STRIPE_SECRET_KEY`
- Plan limits enforced via `config/plans.js` and `services/usage.js`
- Audit logging via onResponse hook → `audit_log` table
...
- 10 migration files (001–010): core schema, auth, TEE sync/submission, fees, portal, superadmin, onboarding/tenants, audit_log, tenant backfill
- Key tables: tenants, users, clients, projects, properties, ekdosi, documents, approvals, workflow_logs, emails, fee_calculations, audit_log, portal_*
...
- `backend/src/app.js` — Fastify app builder (plugin registration, route loading)
- `backend/src/config/plans.js` — SaaS plan limits
- `backend/src/hooks/tenant.js` — Tenant resolution hook
- `backend/src/hooks/audit.js` — Audit logging hook
- `backend/src/routes/admin.js` — Superadmin panel routes
- `backend/src/routes/billing.js` — Stripe billing (SaaS only)
- `.github/workflows/ci-cd.yml` — CI/CD pipeline
- `docs/v2-architecture/` — Full v2 architecture specs and review reports
