...
export async function up(knex) {
  ...
  await knex.raw('CREATE INDEX idx_tenants_stripe ON tenants(stripe_customer_id) WHERE stripe_customer_id IS NOT NULL');

  // ── users: tenant_id + auth token fields ──────────────────────────
  ...
  await knex.raw('CREATE INDEX idx_users_email_verify ON users(email_verify_token) WHERE email_verify_token IS NOT NULL');
  await knex.raw('CREATE INDEX idx_users_pwd_reset ON users(password_reset_token) WHERE password_reset_token IS NOT NULL');
